...
export class BillComponent extends URLLoader implements OnInit {
  invoices$
  id = 0
  fields
  ...
  getFields(language) {
    switch (language) {
      case 'fr':
        this.getFieldsJSON(URLS.FIELDS_BILLS_TABLE_FR)
        break;

      case 'en':
        this.getFieldsJSON(URLS.FIELDS_BILLS_TABLE_EN)
